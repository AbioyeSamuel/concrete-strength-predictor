...
# Step 4: Hyperparameter Tuning for Lasso Regression
param_grid = {
    'alpha': np.logspace(-3, 2, 20),  # Regularization strength
    'max_iter': [10000]   # Maximum iterations
}

lasso_model = Lasso(random_state=42)
grid_search = GridSearchCV(estimator=lasso_model, param_grid=param_grid, cv=5, n_jobs=-1, scoring='r2')
...
cv_scores = cross_val_score(best_lasso_model, X_train_scaled, y_train, scoring='r2', cv=kf)
...
predicted_strength = best_lasso_model.predict(user_input_scaled)
print(f'Predicted Compressive Strength: {predicted_strength[0]}')
